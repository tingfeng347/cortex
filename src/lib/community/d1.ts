import { getDB, d1Query, d1First, d1Run } from "@/lib/db";
...
export async function getQuestionById(id: number): Promise<CommunityQuestionRow | null> {
  return d1First<CommunityQuestionRow>("SELECT * FROM community_questions WHERE id = ?", [id]);
}
...
export async function getUserVote(questionId: number, ipAddress: string): Promise<VoteRow | null> {
  return d1First<VoteRow>("SELECT * FROM question_votes WHERE question_id = ? AND ip_address = ?", [
    questionId,
    ipAddress,
  ]);
}
...
export async function upsertVote(
  ...
  if (!existing) {
    await d1Run("INSERT INTO question_votes (question_id, ip_address, vote) VALUES (?, ?, ?)", [
      questionId,
      ipAddress,
      vote,
    ]);
  ...
  await d1Run("UPDATE question_votes SET vote = ?, updated_at = datetime('now') WHERE id = ?", [
    vote,
    existing.id,
  ]);
...
export async function getAllAdmins(): Promise<AdminRow[]> {
  return d1Query<AdminRow>(
    "SELECT id, username, role, nickname, created_at FROM admins ORDER BY id",
  );
}
...
export async function createAdmin(
  ...
  await d1Run("INSERT INTO admins (username, password_hash, role, nickname) VALUES (?, ?, ?, ?)", [
    username,
    passwordHash,
    role,
    nickname ?? null,
...
export async function updateAdmin(
  id: number,
  data: { passwordHash?: string; role?: string; nickname?: string | null },
): Promise<void> {
  const sets: string[] = [];
...
export async function getAdminByUsernameOrNickname(value: string): Promise<AdminRow | null> {
  return d1First<AdminRow>("SELECT * FROM admins WHERE username = ? OR nickname = ?", [
    value,
    value,
  ]);
}
...
export async function createSession(sessionId: string, adminId: number): Promise<void> {
  await d1Run("INSERT INTO admin_sessions (id, admin_id) VALUES (?, ?)", [sessionId, adminId]);
}
